...
namespace Oop3.PL
{
    public partial class Form1 : Form
    {
        ...
        {
            _personTypes = new List<Type>();
            ddlPersonType.Items.Clear();
            
            var pluginTypes = PluginLoader.Instance.LoadAllPersonTypes();

            foreach (var personType in pluginTypes)
            {
                if (!_personTypes.Contains(personType))
                {
                    _personTypes.Add(personType);
                }
            }
        ...
        private void btnSaveKingdom_Click(object sender, EventArgs e)
        {
            ...
            if (result == DialogResult.OK)
            {
                ...
                try
                {
                    ...
                    {
                        serializedKingdom = _aesEncryptionUtility.Encrypt(serializedKingdom);
                    }
        ...
        private void btnLoadKingdom_Click(object sender, EventArgs e)
        {
            ...
            if (result == DialogResult.OK)
            {
                ...
                try
                {
                    ...
                    var deserializedKingdom = XmlHelper.XmlDeserializeFromString<List<Person>>(fileString, _personTypes.ToArray());
